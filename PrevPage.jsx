...
function justDoPrev() { // Make previous window active, and select left Frame
	curr = app.activeWindow.activePage;
	newPg = app.activeWindow.parent.pages[Math.max(0,curr.documentOffset-1)];
	app.activeWindow.activePage = newPg;
	app.activeWindow.select(getBodyFrame(newPg, 1));	// Select first textframe
}

// Main function: 
function main() {
    if (app.selection.length == 0) { // Nothing selected, so just do prev.
		justDoPrev();
        return;  
    }
    ...
    for(ii = 0; ii < app.selection.length; ii++) {  // for each item in selection
        ...
        if (selFrame == null || selFrame.label.substring(4,0) != "body") {     // e.g. Illustration group is selected
			...
			try {
				...
				newPg = app.activeWindow.parent.pages[Math.max(0,pg.documentOffset-1)];
				app.activeWindow.activePage = newPg;
